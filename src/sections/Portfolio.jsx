...
const cardWithoutDescriptionLandscape = `
w-full
col-span-2
aspect-video
...
const cardWithoutDescriptionPortrait = `
w-full
aspect-[9/16]
bg-white
`;
...
rounded-[var(--rounded-xsmall)]
border-[var(--border-main)]
bg-[var(--bg-color-primary)]
backdrop-blur-[var(--blur-small)]
text-[length:var(--font-size-xsmall)]
text-[var(--font-color-primary)]
shadow-[var(--shadow-big)]
whitespace-nowrap
...
const getProjectGridUnits = (project) => {
    const hasDescription = Boolean(project.hasDescription);
    const isPortrait = project.orientation === "portrait";

    if (!hasDescription && !isPortrait) {
        return 2;
    }
...
const getVisibleProjectsByRows = (items, visibleRows) => {
    ...
    for (const project of items) {
        ...
        if (usedUnits + projectUnits > maxUnits) {
            break;
        }
...
const Portfolio = ({
    ...
    const [activeCategory, setActiveCategory] = useState(
        showFilters ? projectCategories[0]?.key || "" : ""
    );
    ...
    const visibleProjects = useMemo(() => {
        return getVisibleProjectsByRows(sortedProjects, visibleRows);
    ...
    const handleLoadMore = () => {
        setVisibleRows((prevRows) => prevRows + LOAD_MORE_ROWS);
    };
    ...
    const renderCompactOverlay = (project) => (
        <div className={mediaTop}>
            {renderBadges(project)}
        </div>
    );

    const renderMedia = (project, isCompact) => {
        const isPortrait = project.orientation === "portrait";
        const mediaRatio = isCompact
            ? mediaOnly
            : isPortrait
                ? mediaPortrait
                : mediaLandscape;
        ...
        if (project.type === "video") {
            return (
                <div
                    className={`${mediaWrap} ${mediaRatio}`}
                ...
                >
                    ...
                    <div className={visualOverlay}>
                        <div className={overlayIcon}>
                            {renderOverlayIcon(project.type)}
                        </div>
                    </div>
        ...
        if (project.type === "audio") {
            return (
                <div
                    className={`${audioWrap} ${mediaRatio}`}
                    onClick={() => openModal(project)}
                >
                    <img
                        src={project.poster}
        ...
        if (modalItem.type === "video") {
            return (
                <video
                    className={modalMedia}
                    src={modalItem.src}
                    poster={modalItem.poster || undefined}
                    controls
                    autoPlay
                    playsInline
                    preload="metadata"
            ...
            <section className={container}>
                ...
                {visibleProjects.length === 0 ? (
                    <p className={emptyText}>
                        فعلاً نمونه‌کاری در این دسته‌بندی ثبت نشده است.
                    </p>
                ) : (
                    <>
                        <div className={grid}>
                            {visibleProjects.map((project) => {
                                ...
                                const cardClass = hasDescription
                                    ...
                                    : `${cardBase} ${isPortrait
                                        ? cardWithoutDescriptionPortrait
                                        : cardWithoutDescriptionLandscape
                                    }`;
                        ...
                        {hasMoreProjects && (
                            <>
                                ...
                                <p className={loadMoreInfo}>
                                    {remainingProjectsCount} نمونه‌کار دیگر باقی مانده است.
                                </p>
                            </>
                        )}
                    </>
